...
| PR #21 production trust-boundary remediation | `d99ca2d`..`HEAD`; PR #21 (DRAFT, unmerged) | Enforced distinct Production/Testing pins, machine-owned release mode with caller assertion only, C# machine trust snapshot and loader (`AgentMachineTrustConfiguration`/`MachineAgentTrustConfigurationLoader`), startup rejection of obsolete `PosAgent:ReleaseChannel`, file/owner/ancestor control-file trust, actual LocalSystem CNG key-file ACL evidence, and generated operation-ID audit/timeline correlation. Added 37 focused C# trust loader/composition tests (399 total POS tests) and 153 Pester tests. |
