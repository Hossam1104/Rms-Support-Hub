...
1. Write a throwaway introspection script (put it in the scratchpad, not the repo) that connects to server `10.10.8.181`, database `RmsMainTest2`, using credentials from user-secrets/environment variables (never hardcoded in the script), driver `ODBC Driver 17 for SQL Server`, reusing the driver-fallback connection-string pattern from `modules/flat_order.py:40-85`. Run `SELECT TOP 1 *` against each of: `Consumers`, `LoyaltyConsumerAddresses`, `OrderRequests`, `RequestOrderHeaders`, `RequestOrderDetails`, `RequestOrderTransactions`, `Invoices`. Print each table's column names.
