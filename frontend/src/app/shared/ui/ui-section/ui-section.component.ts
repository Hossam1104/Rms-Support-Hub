...
@Component({
  ...
  template: `
    <section class="ui-section" [class.is-collapsed]="!expandedState()" [class.is-disabled]="disabled()">
      <header class="ui-section__header">
        <button
          ...
          (click)="toggle()">
          <span class="ui-section__marker" [class.is-complete]="completed()" [attr.aria-label]="completed() ? 'Complete' : 'Incomplete'">
            <i class="bi" [class.bi-check-lg]="completed()" [class.bi-dash]="!completed()"></i>
          </span>
