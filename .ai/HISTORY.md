...
| WPF-01 Sol security remediation | Local validation on `feat/wpf-01-shared-agent-local-ipc`; Draft PR #32 remains Draft | Closed S01-S09 with least-privilege operator ACL, explicit NETWORK deny, LocalSystem server-token verification, local-only group resolution, truthful invocation propagation, strict correlation matching, source-coherent authorization, stable AccessControl 5.0.0, and fail-closed diagnostic audit semantics. POS build and tests pass; no WPF UI, SignalR, Production, native RMS, or customer database changes. |
