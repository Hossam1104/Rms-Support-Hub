...
| INT-06I - UAC-safe Administrator authorization and Scalar/OpenAPI documentation | `int-06i-admin-auth-scalar`; POS Release validation and generated artifacts | Replaced UAC token-elevation membership coupling with OS-backed local-group resolution including indirect membership and well-known Administrators SID comparison; kept claims test-only; added non-production Scalar/OpenAPI with complete operation/schema metadata; Agent 85/85, frontend 341/341, deterministic generation, package audit, and WinUI publish passed. Post-remediation Chrome/Edge rerun is not claimed because no connected browser backend was available; PR remains unmerged pending independent security review; INT-07 was not executed. |
