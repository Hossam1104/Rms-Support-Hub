...
- Connection: server `10.10.8.181`, credentials supplied via .NET user-secrets / environment variables (see `README.md` "Configuration & secrets") — never committed here, same connection for both environments.
...
- Give UPC two configs — `RmsMainProd` and `RmsMainTest2`, server `10.10.8.181`, credentials from user-secrets/environment variables (never hardcoded), driver `ODBC Driver 17 for SQL Server` — with env-var overrides (`UPC_ECOM_PROD_*` / `UPC_ECOM_TEST_*`) so real creds remain config-only (matching the existing `_db_config` env-prefix convention, `modules/db_config.py:11-18`). Assign prod config to `UPC Production`, test config to `UPC Testing`.
