...
- `WPF-06`: Database backup creation, principal-scoped bounded inventory, and one shared Agent-owned local artifact-delivery/export path for database backups and Support Bundles are implemented on the current Draft PR branch for primary story #13034. Database restore is deliberately not implemented and remains separately gated; #13034 is Active/P1 until restore governance is reconciled.
