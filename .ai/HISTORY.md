...
| INT-04 - Agent host / runtime composition | 2026-08-11: destination POS solution restore/build; Domain 7/7, Application 75/75, Infrastructure 60/60, Agent 57/57; WinUI `win-x64` publish; boundary/static checks; memory checks | Composed a headless ASP.NET Core Windows Service-capable Agent from POS SHA `010abc52dc110cfde3dc2c53e057890ff6edaf97` at fixed HTTPS origin `https://rms-pos-agent.localhost:5001`, with HTTP/1.1 loopback, production Negotiate, SID/local-Administrators authorization, exact-origin CORS/Origin, mutation-token and service-owned storage foundations, and only health/session routes. Support Hub backend/frontend, POS Angular, OpenAPI, and feature operations remain unchanged/out of scope. |
| INT-05 - Browser transport / OpenAPI / client adapter | 2026-08-11 validation | Added the Agent OpenAPI/client seam and direct `HttpBackend` transport; POS Release build and Agent 69/69 tests passed. Accepted after INT-05F; feature operations, POS UI activation, and live evidence remain excluded. |
| INT-05F - OpenAPI generator peer-dependency isolation | 2026-08-11 validation | Plain frontend/tool `npm ci`, deterministic two-pass generation, frontend 341/341, POS Release build 0 warnings, Agent 69/69, and production audit passed. Isolated `openapi-typescript@7.13.0` with TypeScript 5.9.3 and removed the global peer bypass. |
| INT-CI01 - Portable Application Linux CI baseline remediation | `c560d97`; GitHub Actions run `31540243375` | Replaced host-dependent `System.IO.Path` use with deterministic Windows maintenance/SMB path semantics, made downloader timing fixtures manual-clock based, and resolved the nine pre-existing Ubuntu Application failures. Domain 7/7, Application 76/76, Infrastructure 60/60, Agent 69/69, Release build 0 warnings, all five POS CI lanes green; INT-06 remains owner-gated. |
| INT-06H - Real browser runtime evidence | `e959a45` checkpoint; sanitized evidence in `docs/evidence/POS_INT06_LIVE_TRANSPORT_EVIDENCE.md` | Actual Chrome and Edge LNA allow/block, direct Agent, Negotiate/session, SID non-exposure, and cleanup evidence collected. Blocked at normal-browser local-Administrator authorization mismatch; no runtime source or POS feature changed; INT-07 not executed. |
| INT-06I - UAC-safe Administrator authorization and Scalar/OpenAPI documentation | `int-06i-admin-auth-scalar`; POS Release validation and generated artifacts | Replaced UAC token-elevation membership coupling with OS-backed local-group resolution including indirect membership and well-known Administrators SID comparison; kept claims test-only; added non-production Scalar/OpenAPI with complete operation/schema metadata; PR remains unmerged pending independent security review; INT-07 was not executed. |
| INT-06I-F1 - Scalar contract accuracy and live browser closure | 2026-08-13; sanitized evidence in `docs/evidence/POS_INT06_LIVE_TRANSPORT_EVIDENCE.md` | Corrected framework-versus-endpoint response contracts, added runtime shape and route/OpenAPI parity guards, enabled local-only Scalar rendering under a route-scoped CSP, and proved Production Kestrel document-route isolation plus normal Chrome/Edge authorization. Agent 95/95, frontend 341/341, deterministic generation, audits, and WinUI publish passed; PR remains open/draft/unmerged for independent security review; INT-07 was not executed. |
