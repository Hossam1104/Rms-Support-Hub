...
@Component({
  ...
  `,
  styles: [`
    .prompt-studio { min-height: 100vh; padding: calc(var(--navbar-height) + var(--space-7)) var(--space-6) var(--space-8); background: var(--surface-page); }
    ...
    .prompt-studio__header::after { content: ''; position: absolute; bottom: -1px; left: 0; width: 120px; height: 2px; border-radius: var(--radius-pill); background: var(--grad-brand); }
    .prompt-studio__eyebrow { margin: 0 0 var(--space-3); color: var(--text-accent); font-size: var(--text-xs); font-weight: var(--weight-bold); text-transform: uppercase; }
    .prompt-studio__header h1 { margin: 0; color: var(--text-primary); font-size: 2.6rem; font-weight: var(--weight-heavy); line-height: 1.08; }
    ...
    .prompt-studio__grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: var(--space-5); }
    .prompt-studio__grid > app-tool-card { opacity: 0; animation: prompt-card-in var(--d-slow) var(--ease-out) forwards; }
    ...
    @media (max-width: 680px) {
      ...
      .prompt-studio__header h1 { font-size: 2rem; }
      .prompt-studio__grid { grid-template-columns: 1fr; gap: var(--space-4); }
    }
