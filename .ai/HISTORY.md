...
| Milestone | Evidence | Outcome |
|---|---|---|
| WPF-05 Logs, safe diagnostic evidence, and Support Bundle | Branch `feat/wpf-05-logs-support-bundle`; focused Application 6/6, WPF view-model 5/5, WPF IPC 4/4, Agent Integration 1/1, existing Support Bundle regressions 5/5; full Release 578/578; PowerShell 37/37; Pester 172/172 | Added bounded redacted Logs/evidence through the shared Agent/Application seam and typed `rms.logs.evidence` Local IPC, plus administrator-only typed `support.bundle.generate` Support Bundle metadata with principal binding, audit/revocation, fixed roots, and no WPF artifact export. Exact Release runtime PID 9028 is responding; Computer Use was unavailable and visual evidence is not claimed. |
