...
| INT-05 - Browser transport / OpenAPI / client adapter | 2026-08-11: POS solution Release build (0 warnings/errors), Domain 7/7, Application 75/75, Infrastructure 60/60, Agent 69/69, WinUI publish (exe + 15 `.pri` + 33 `.xbf`), frontend 341/341 across 56 files, deterministic two-pass OpenAPI/TypeScript generation, and static boundary checks | Added the Agent-owned versioned build-time OpenAPI contract and IntegrationTest-only runtime route, server-owned mutation operation registry/token issuance seam, portable safe DTOs, host-before-correlation ordering, Support Hub-owned `openapi-typescript@7.13.0` generated types, isolated `HttpBackend` transport/error classification, and pinned CI drift validation. No feature operations, POS UI activation, live browser/device evidence, or Production access. |
