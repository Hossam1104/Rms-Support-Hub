...
- **Query**: `q` (alias for `orderNumber`), `orderNumber`, `exactMatch` (optional; defaults to `true`), `phone`, `branchCode`, `status` (single value, 1–9), `statuses` (repeated, e.g. `?statuses=6&statuses=7` — multi-select status chips; takes precedence over `status` when both are given), `succeeded` (bool), `hasException` (bool), `dateFrom`, `dateTo`, `page` (default 1), `pageSize` (default 25, clamped to ≤200), `sort` (`order_date`\|`net_total`\|`item_count`, optionally prefixed `-`/`+`; default `order_date DESC` for filtered results). With no filters, the dashboard intentionally returns only the ten newest `OrderRequests` by `Id DESC`.
